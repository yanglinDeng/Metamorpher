...
class Evaluator():
    @classmethod
    def input_check(cls, imgF, imgA=None, imgB=None): #检查图像数据类型以及维度是否有错误
        if imgA is None:
    ...
    def EN(cls, img):  # entropy
        cls.input_check(img)
        a = np.uint8(np.round(img)).flatten()#把多维数组展平成一维数组
    ...
    def SSIM(cls, image_F, image_A, image_B):
        cls.input_check(image_F, image_A, image_B)
        return ssim(image_F,image_A)+ssim(image_F,image_B)#计算两幅图像

    @classmethod
    def SSIM_ir(cls, image_F, image_A, image_B):
        cls.input_check(image_F, image_A, image_B)
        return ssim(image_F,image_A)#计算两幅图像

    @classmethod
    def SSIM_vi(cls, image_F, image_A, image_B):
        cls.input_check(image_F, image_A, image_B)
        return ssim(image_F,image_B)#计算两幅图像
